...
class FlightRouteListAdapter(private val context: Context,private val routeList: List<OperationalFlights>):
    ...
    override fun onBindViewHolder(holder: MovieViewHolder, position: Int) {
        ...
        val sourceDestination =
            routeData.flightLegs?.first()?.departureInformation?.airport?.city?.name + " - " +
                    routeData.flightLegs?.first()?.arrivalInformation?.airport?.city?.name
        holder.routeTitle.text = sourceDestination
        holder.routeTime.text = context.getString(
            R.string.from_to,
            routeData.flightLegs?.first()?.departureInformation?.times?.scheduled,
            routeData.flightLegs?.first()?.arrivalInformation?.times?.scheduled
        )
