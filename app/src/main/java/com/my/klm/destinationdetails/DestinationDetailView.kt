...
class DestinationDetailView : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        destinationDetailData?.let {
            ...
            val tempData = getString(R.string.degree_symbol, destinationDetailData.weather.data.temperature.first().value.toString(), destinationDetailData.weather.data.temperature.first().unit)
            temp_value.text = tempData
            currency_value.text = destinationDetailData.currency.data.label
            spoken_lang_values.text = destinationDetailData.spokenLanguages.first().label
